-- =============================================================
-- KAN14 Fase 3 - Lote 02 (Académico-Operativo) - APPLY
-- Objetivo: migrar columnas de fecha a timestamptz de forma segura.
-- Requisito: ejecutar precheck y confirmar total_invalidos_lote02 = 0.
-- =============================================================
...
BEGIN
    ...
    LOOP
        ...
        IF v_type IS NULL THEN
            RAISE NOTICE 'KAN14 lote02: %.% no existe; se omite.', r.tabla, r.columna;
            CONTINUE;
        END IF;
        ...
        IF v_type IN ('text', 'character varying') THEN
            ...
            IF COALESCE(v_invalid, 0) > 0 THEN
                RAISE EXCEPTION 'KAN14 lote02 abortado: %.% tiene % valores no convertibles.', r.tabla, r.columna, v_invalid;
            END IF;
            ...
            RAISE NOTICE 'KAN14 lote02: %.% convertido a timestamptz.', r.tabla, r.columna;
        ELSIF v_type = 'timestamp without time zone' THEN
            ...
            RAISE NOTICE 'KAN14 lote02: %.% timestamp->timestamptz (UTC) aplicado.', r.tabla, r.columna;
        ELSE
            RAISE NOTICE 'KAN14 lote02: %.% ya no es text/varchar/timestamp sin zona (tipo=%), se omite.', r.tabla, r.columna, v_type;
        END IF;
    END LOOP;
END;
$$;
