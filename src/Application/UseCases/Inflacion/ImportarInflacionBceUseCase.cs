...
public sealed class ImportarInflacionBceUseCase(
    IServicioImportacionBCE servicioImportacionBce,
    ...
    IAuditoriaServicio auditoriaServicio)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Trace.WriteLine($"[{DateTime.UtcNow:O}] InflacionImportBCE: inicio usecase. url={solicitud.Url}, usuario={usuarioId}");

        var lectura = await servicioImportacionBce.ObtenerInflacionAnualAsync(solicitud, cancellationToken);
        ...
        catch (Exception ex)
        {
            ...
            Trace.WriteLine($"[{DateTime.UtcNow:O}] InflacionDBError: import bce guardado falló. detalle={detalle}");
        ...
        try
        {
            await auditoriaServicio.RegistrarAsync(
                ...
                entidadNombre: "ImportacionBCE",
                entidadId: DateTime.UtcNow.Ticks.ToString(),
                accionNombre: "IMPORTAR_BCE",
                resumenTexto: $"Importación BCE completada. Correctas: {filasCorrectas}, omitidas: {omitidos.Count}, errores: {errores.Count}.",
        ...
        Trace.WriteLine($"[{DateTime.UtcNow:O}] InflacionImportBCE: fin usecase. procesadas={lectura.Filas.Count + lectura.Errores.Count}, correctas={filasCorrectas}, omitidas={omitidos.Count}, errores={errores.Count}");
