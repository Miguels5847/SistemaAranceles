...
public interface IRepositorioServicioMantenimiento
{
    ...
    Task<IReadOnlyList<ServicioMantenimiento>> ListarPorCarreraAsync(int carreraId, TipoRubroMantenimiento? tipo = null, CancellationToken ct = default);
    Task<decimal> SumarCostoAnualPorTipoAsync(int carreraId, TipoRubroMantenimiento tipo, CancellationToken ct = default);
