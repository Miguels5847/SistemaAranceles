...
internal sealed class ConfiguracionActivoFijo : IEntityTypeConfiguration<ActivoFijo>
{
    public void Configure(EntityTypeBuilder<ActivoFijo> builder)
    {
        ...
        builder.Property(x => x.TipoCalculoCantidad).HasColumnName("tipo_calculo_cantidad").HasConversion<string>().HasMaxLength(30).IsRequired();
        builder.Property(x => x.FactorMultiplicador).HasColumnName("factor_multiplicador").HasColumnType("numeric(18,4)").IsRequired();
        builder.Property(x => x.OffsetCantidad).HasColumnName("offset_cantidad").HasColumnType("numeric(18,4)").IsRequired();
...
internal sealed class ConfiguracionCatalogoActivoBase : IEntityTypeConfiguration<CatalogoActivoBase>
{
    public void Configure(EntityTypeBuilder<CatalogoActivoBase> builder)
    {
        builder.ToTable("catalogo_activo_base");

        builder.Property(x => x.Id).HasColumnName("id");
        builder.Property(x => x.Descripcion).HasColumnName("descripcion").HasMaxLength(200).IsRequired();
        builder.Property(x => x.Categoria).HasColumnName("categoria").HasConversion<string>().HasMaxLength(40).IsRequired();
        builder.Property(x => x.TipoCalculoCantidad).HasColumnName("tipo_calculo_cantidad").HasConversion<string>().HasMaxLength(30).IsRequired();
        builder.Property(x => x.CantidadDefault).HasColumnName("cantidad_default").HasColumnType("numeric(18,4)").IsRequired();
        builder.Property(x => x.UnidadMedida).HasColumnName("unidad_medida").HasMaxLength(20).IsRequired();
        builder.Property(x => x.ValorUnitario).HasColumnName("valor_unitario").HasColumnType("numeric(18,2)").IsRequired();
        builder.Property(x => x.FactorMultiplicador).HasColumnName("factor_multiplicador").HasColumnType("numeric(18,4)").IsRequired();
        builder.Property(x => x.OffsetCantidad).HasColumnName("offset_cantidad").HasColumnType("numeric(18,4)").IsRequired();
